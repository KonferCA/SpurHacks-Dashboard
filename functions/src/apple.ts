...
export const createTicket = onCall(async (request: CallableRequest) => {
  ...
  try {
          ...
          backFields: [
            {
              ...
              label: "More Info",
              value:
                "For more details, visit our website at spurhacks.com",
            },
            {
    ...
    const icon2xBuffer = fs.readFileSync(path.join(assetsDir, "spur-icon@2x.png"));
    const stripBuffer = fs.readFileSync(path.join(assetsDir, "dark-theme", "strip.png"));
    const strip2xBuffer = fs.readFileSync(path.join(assetsDir, "dark-theme", "strip@2x.png"));
    ...
    await fileRef.save(buffer, { metadata: { contentType: "application/vnd.apple.pkpass" } });
